...
// implementati export (se vor extrage parintii cu copiii lor de forma GET / care va intoarce in body un 
// array de university 
// unde university contine un array de students si courses plus enrollements ca perechi de id student si course

application.get('/',async(request,response,next)=>{
  try{
    const arrayUniversitati=[]
    const universities = await University.findAll();
    for(let u of universities){

      const obUniversitate = {};
      obUniversitate["id"] = u.id;
      obUniversitate["name"] = u.name;
      obUniversitate["students"] = await u.getStudents();
      obUniversitate["courses"] = await u.getCourses();
      const inrolari = [];
      for(let s of obUniversitate["students"]){
        const ob = {};
        const cursuri = await s.getCourses();
        for(let c of cursuri){
          ob["idStudent"] = s.id;
          ob["idCourse"] = c.id;
          inrolari.push(ob);
        }
      }
      obUniversitate["enrollment"] = inrolari;
      arrayUniversitati.push(obUniversitate)
    } 
    response.json(arrayUniversitati)
  }catch(error){
    next(error)
  }
})