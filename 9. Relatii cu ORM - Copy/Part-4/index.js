...
// implementați cererea get pentru lista de cursuri a unui student 
// (GET /universities/:universityId/students/:studentId/enrollements)

application.get('/universities/:universityId/students/:studentId/enrollements',async(request,response,next)=>{
  try{
    const university = await University.findByPk(request.params.universityId);
    if(university){
      const students = await university.getStudents({where:{id: request.params.studentId}});
      const student = students.shift();
      if(student){
